...
请写下您珍贵的回忆..."
      ...
      <div class="bottom-section">
        <label class="agreement-checkbox">
          ...
          <span class="agreement-text" @click="showAgreementModal">
            <span class="text-en">Agree to the relativistic arbitrage agreement</span>
            <br/>
            <span class="text-zh">同意相对论套利协议</span>
          </span>
        </label>
    ...
    <!-- 协议模态框 -->
    <div v-if="showModal" class="modal-overlay" @click="closeAgreementModal">
      <div class="agreement-modal" @click.stop>
        <div class="modal-header">
          <h2 class="modal-title">
            <span class="text-en">Relativistic Arbitrage Agreement</span>
            <span class="text-zh" style="font-size: 14px; color: rgba(255, 255, 255, 0.6);">相对论套利协议</span>
          </h2>
          <button class="close-button" @click="closeAgreementModal">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="18" y1="6" x2="6" y2="18"/>
              <line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
        ...
        <div class="agreement-content">
          <div class="agreement-section">
            <h3>
              <span class="text-en">一、Core Statement of the Agreement 协议核心声明</span>
            </h3>
            
            <p>
              <span class="text-en"><strong>Protocol nature:</strong> This protocol is a decentralized time asset trading protocol based on the physical effects of special relativity and general relativity, aiming to convert the unique 38μs/day relativity time surplus in space into tradable digital assets (STIME tokens) through the difference in gravitational force between the satellite and the ground, providing users with a physical path to break free from the consensus time constraint of Earth. This agreement is not a financial product, but a derivative application of relativistic physical effects, possessing both philosophical significance and technical realization. All transaction behaviors are based on the physical rights granted by the laws of the universe and do not rely on the endorsement of any centralized institution.</span>
              <span class="text-zh"><strong>协议性质：</strong>本协议是基于狭义相对论与广义相对论物理效应构建的去中心化时间资产交易协议，旨在通过卫星与地面的引力差，将太空独有的38μs/日相对论时间盈余转化为可交易的数字资产（STIME代币），为用户提供挣脱地球共识时间奴役的物理路径。本协议并非金融理财产品，而是一项兼具哲学意义与技术实现的物理实验衍生应用，所有交易行为均基于宇宙法则赋予的物理权利，不依赖任何中心化机构背书。</span>
            </p>
            
            <p>
              <span class="text-en"><strong>Core Asset Definition:</strong> STIME tokens (relativistic time coins) are the core assets of this protocol. Each token corresponds to the ownership of 1 microsecond of time that is not accounted for in the Earth's consensus time system but actually passes in the weak gravitational environment of geosynchronous orbit satellites. The generation of this asset is rigidly guaranteed by the relativistic effect, with a fixed daily output of 38 tokens, which cannot be forged or minted further, endowing it with absolute scarcity and physical anchor attributes.</span>
              <span class="text-zh"><strong>核心资产定义：</strong>STIME代币（相对论时间币）是本协议的核心资产，每一枚代币对应在地球共识时间体系中不存在、但在地球同步轨道卫星弱引力环境下真实流逝的1微秒时间所有权。该资产的生成由相对论效应刚性保障，每日固定产量为38枚，不可伪造、不可增发，具备绝对稀缺性与物理锚定属性。</span>
            </p>
            
            <p>
              <span class="text-en"><strong>User Commitment:</strong> The user confirms that they fully understand the physical principle of relativistic time difference, the asset generation mechanism, and the memory-erasure cost mechanism associated with the transaction, and voluntarily participates in asset transactions under this agreement. The user acknowledges that the essence of the transaction is to exchange existing existence for free time in space, and voluntarily accepts the direct consequence of "the permanent erasure of my personal digital files uploaded to the platform" without any objection.</span>
              <span class="text-zh"><strong>用户承诺：</strong>用户确认已充分理解相对论时间差的物理原理、资产生成机制及交易伴随的记忆抹除代价机制，自愿参与本协议下的资产交易；认可交易本质是以既有存在换取太空自由时间的套利行为，自愿接受"本人主动上传至平台的个人数字档案被永久抹除"的直接影响，无任何异议。</span>
            </p>
          </div>
          
          <div class="agreement-section">
            <h3>
              <span class="text-en">二、Transaction Consideration Mechanism 交易代价机制</span>
            </h3>
            
            <p>
              <span class="text-en"><strong>Memory Erasure Cost:</strong> For every 1 microsecond of STIME tokens traded, a personal digital file (including but not limited to photos, diaries, manuscripts, etc.) actively uploaded to the trading platform will be permanently and irreversibly deleted through the application background system. The erasure log is synchronized to the platform in real time, and users can view the cumulative amount of sacrificed memories and related records. This mechanism embodies the core philosophy that arbitrage necessitates the cost of depriving existing existence.</span>
              <span class="text-zh"><strong>记忆抹除代价：</strong>每成交1微秒对应的STIME代币，用户的主动上传至本交易平台的一段个人数字档案（包括但不限于照片、日记、手稿等）将通过APP后台系统进行永久、不可逆的物理抹除。抹除日志实时同步至平台，用户可查看累计献祭记忆数量及相关记录。该机制体现套利需以剥夺既有存在为代价的核心哲学。</span>
            </p>
          </div>
        </div>
        
        <div class="modal-actions">
          <button class="modal-button disagree-button" @click="handleDisagree">
            <span class="text-en">Disagree</span>
            <span class="text-zh" style="font-size: 12px;">不同意</span>
          </button>
          <button class="modal-button agree-button" @click="handleAgree">
            <span class="text-en">Agree</span>
            <span class="text-zh" style="font-size: 12px;">同意</span>
          </button>
        </div>
      </div>
    </div>
...
const canUpload = computed(() => {
  return uploadedFiles.value.length > 0 && agreedToTerms.value
})
...
const showAgreementModal = (event) => {
